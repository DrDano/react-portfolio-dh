...
const pinnedProjects = async () => {
    ...
    const readmeArr = pinnedData.user.pinnedItems.edges.map((project) => {
        return {name: project.node.name, readme: project.node.object.text, url: project.node.url};
    });
    ...
    const portfolioData = {
        ...
        photo: "https://raw.githubusercontent.com" + readmeArr[0].url.split("/").filter((chunk)=>chunk==="github").join("") + "/DrDano/" + readmeArr[0].name + "/main/src/assets/images/" + readmeArr[0].name + ".jpg",
    }
